...
print(f"sub category : {train.loc[1, 'Sub Category']}")
...
train["label"] = train["Sub Category"].apply(lambda x: label2id[x])
y_train = tf.keras.utils.to_categorical(train.label, num_classes=len(labels))

test["label"] = test["Sub Category"].apply(lambda x: label2id[x])
...
# model saving
model.save("./sub_category_model", save_format='tf')
print("model saved...")
